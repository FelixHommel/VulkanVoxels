...
void Model::Builder::loadModel(const std::filesystem::path& filepath)
{
    ...
    for(const auto& s : shapes)
    {
        for(const auto& i : s.mesh.indices)
        {
            ...
            {
                v.normal = {
                    attrib.normals[(3 * static_cast<std::size_t>(i.normal_index)) + 0],
